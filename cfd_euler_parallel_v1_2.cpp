...
int main(){
    ...
    std::atomic<int> oper_count(0);
    /*When counting operations: Multiple access to same data neglected.*/
    #pragma omp parallel for
    {
        for (int i = 0; i < Nx+2; i++){
            for (int j = 0; j < Ny+2; j++){
                ...
                oper_count.fetch_add(1, std::memory_order_relaxed);
            }
        }
    }
    auto t2_init = std::chrono::high_resolution_clock::now();
    double elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(t2_init-t1_init).count();
    double elapsed_in_sec = elapsed / 1000.0;

    int total_ops = oper_count.load(std::memory_order_relaxed) * 5 * sizeof(double); /* 5 array(double) write operations for each iteration*/
    double total_ops_GB = total_ops / (1024.0*1024.0*1024.0);

    double gb_per_sec = total_ops_GB / elapsed_in_sec;


    std::cout << "The initialization process took "
              << total_ops
    ...
    std::cout << "The initialization process took "
              ...
              << " milliseconds. \n";
    if(elapsed_in_sec == 0){
    std::cout << "The initialization bandwidth could not be determined due to the amount of time passed (0). \n";
    }
    else{
    std::cout << "The initialization had the bandwidth: "
     << gb_per_sec
     << "  GB/s. \n";
    }
    ...
    for (int n = 0; n < nSteps; n++){
        ...
        auto t1_tmp = std::chrono::high_resolution_clock::now();
        std::atomic<int> oper_count(0);
        #pragma omp parallel for
        {
            for (int j = 0; j < Ny+2; j++){
        ...
        auto t2_tmp = std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(t2_tmp-t1_tmp).count();
        double elapsed_in_sec = elapsed / 1000.0;
        int total_ops = oper_count.load(std::memory_order_relaxed) * 4 * sizeof(double); /* 4 array(double) write operations for each iteration*/
        double total_ops_GB = total_ops / (1024.0*1024.0*1024.0);
        double gb_per_sec = total_ops_GB / elapsed_in_sec; 
        std::cout << "The Left boundary process took "
                << total_ops
                << " operations (bytes moved). \n"; 
        std::cout << "The Left boundary process took "
                << elapsed
                << " milliseconds. \n";
        if(elapsed_in_sec == 0){
            std::cout << "The Left boundary process bandwidth could not be determined due to the amount of time passed (0). \n";
            }
            else{
            std::cout << "The Left boundary process had the bandwidth: "
                << gb_per_sec
                << "  GB/s. \n";
            }
        ...
        t1_tmp = std::chrono::high_resolution_clock::now();
        oper_count.store(0);
        #pragma omp parallel for
        {
            for (int j = 0; j < Ny+2; j++){
        ...
        t2_tmp = std::chrono::high_resolution_clock::now();
        elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(t2_tmp-t1_tmp).count();
        elapsed_in_sec = elapsed / 1000.0;
        total_ops = oper_count.load(std::memory_order_relaxed) * 8 * sizeof(double); /* 4-4 array(double) write and read operations for each iteration*/
        total_ops_GB = total_ops / (1024.0*1024.0*1024.0);
        gb_per_sec = total_ops_GB / elapsed_in_sec;
        std::cout << "The Right boundary process took "
                << total_ops
                << " operations (bytes moved). \n"; 
        std::cout << "The Right boundary process took "
                << elapsed
                << " milliseconds. \n";
        if(elapsed_in_sec == 0){
            std::cout << "The Right boundary process bandwidth could not be determined due to the amount of time passed (0). \n";
            }
        else{
            std::cout << "The Right boundary process had the bandwidth: "
                << gb_per_sec
                << "  GB/s. \n";
            }
        ...
        total_ops_GB = total_ops / (1024.0*1024.0*1024.0);
        gb_per_sec = total_ops_GB / elapsed_in_sec;
        std::cout << "The Bottom boundary process took "
                << total_ops
                << " operations (bytes moved). \n"; 
        std::cout << "The Bottom boundary process took "
                << elapsed
                << " milliseconds. \n";
        if(elapsed_in_sec == 0){
            std::cout << "The Bottom boundary process bandwidth could not be determined due to the amount of time passed (0). \n";
            }
            else{
            std::cout << "The Bottom boundary process had the bandwidth: "
                << gb_per_sec
                << "  GB/s. \n";
            }
        ...
        elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(t2_tmp-t1_tmp).count();
        elapsed_in_sec = elapsed / 1000.0;
        total_ops = oper_count.load(std::memory_order_relaxed) * 8 * sizeof(double); /* 4-4 array(double) write read operations for each iteration*/
        total_ops_GB = total_ops / (1024.0*1024.0*1024.0);
        gb_per_sec = total_ops_GB / elapsed_in_sec;
        std::cout << "The Top boundary process took "
                << total_ops
                << " operations (bytes moved). \n"; 
        std::cout << "The Top boundary process took "
                << elapsed
                << " milliseconds. \n";
        if(elapsed_in_sec == 0){
            std::cout << "The Top boundary process bandwidth could not be determined due to the amount of time passed (0). \n";
            }
            else{
            std::cout << "The Top boundary process had the bandwidth: "
                << gb_per_sec
                << "  GB/s. \n";
            }
            ...
            for (int i = 1; i <= Nx; i++){
                for (int j = 1; j <= Ny; j++){
                    ...
                    if (solid[i][j]) {
                        ...
                        #pragma omp atomic
                         oper_count2 += 9; /* 1 (if condition) + 4  data read, 4 write */
                    ...
                    #pragma omp atomic
                        oper_count2 += 20; /* 16 data read, 4 write*/
                    // Compute fluxes
                    ...
                    #pragma omp atomic
                        oper_count2 += 32; /* for each flux function call: 4 data read, 4 write*/
                    ...
                    #pragma omp atomic
                        oper_count2 += 28; /* for each array: 1 write + 6 access. Multiple access to same data neglected. Just as in other counting cases.*/
        ...
        elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(t2_tmp-t1_tmp).count();
        elapsed_in_sec = elapsed / 1000.0;
        total_ops = oper_count2 * 4 * sizeof(double); /* 4 array(double) write operations for each iteration*/
        total_ops_GB = total_ops / (1024.0*1024.0*1024.0);
        gb_per_sec = total_ops_GB / elapsed_in_sec;
        std::cout << "The Interior process took "
                << total_ops
                << " operations (bytes moved). \n"; 
        std::cout << "The Interior process took "
                << elapsed
                << " milliseconds. \n";
        if(elapsed_in_sec == 0){
            std::cout << "The Interior process bandwidth could not be determined due to the amount of time passed (0). \n";
            }
            else{
            std::cout << "The Interior process had the bandwidth: "
                << gb_per_sec
                << "  GB/s. \n";
            }
        ...
        total_ops_GB = total_ops / (1024.0*1024.0*1024.0);
        gb_per_sec = total_ops_GB / elapsed_in_sec;
        std::cout << "The Copy Back process took "
                << total_ops
                << " operations (bytes moved). \n"; 
        std::cout << "The Copy Back process took "
                << elapsed
                << " milliseconds. \n";
        if(elapsed_in_sec == 0){
            std::cout << "The Copy Back process bandwidth could not be determined due to the amount of time passed (0). \n";
            }
            else{
            std::cout << "The Copy Back process had the bandwidth: "
                << gb_per_sec
                << "  GB/s. \n";
            }
        ...
        elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(t2_tmp-t1_tmp).count();
        elapsed_in_sec = elapsed / 1000.0;
        total_ops = oper_count.load(std::memory_order_relaxed) * 7 * sizeof(double); /* 5-2 array(double) write-read operations for each iteration*/
        total_ops_GB = total_ops / (1024.0*1024.0*1024.0);
        gb_per_sec = total_ops_GB / elapsed_in_sec;
        std::cout << "The Kinetic Energy Calculation process took "
                << total_ops
                << " operations (bytes moved). \n"; 
        std::cout << "The Kinetic Energy Calculation process took "
                << elapsed
                << " milliseconds. \n";
        if(elapsed_in_sec == 0){
            std::cout << "The Kinetic Energy Calculation process bandwidth could not be determined due to the amount of time passed (0). \n";
            }
            else{
            std::cout << "The Kinetic Energy Calculation process had the bandwidth: "
                << gb_per_sec
                << "  GB/s. \n";
            }
        ...
        break; //we only look at 1 iteration
    }
    /*
